...
X_aug_non_defect, y_aug_non_defect = load_point_clouds_from_mesh(augmented_non_defect_folder, 0, n_points=n_points,
...
labels_train, counts_train = np.unique(np.argmax(y_train, axis=1), return_counts=True)
print("Labels distribution in training data:", dict(zip(labels_train, counts_train)))

labels_test, counts_test = np.unique(np.argmax(y_test, axis=1), return_counts=True)
print("Labels distribution in test data:", dict(zip(labels_test, counts_test)))
...
# Сохранение модели
model.save('model_dd_2705_2.keras')
...
# Проверка распределения классов в y_test и y_pred_classes
labels_test, counts_test = np.unique(y_true, return_counts=True)
print("Labels distribution in y_test:", dict(zip(labels_test, counts_test)))

labels_pred, counts_pred = np.unique(y_pred_classes, return_counts=True)
print("Labels distribution in y_pred_classes:", dict(zip(labels_pred, counts_pred)))
